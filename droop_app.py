...
Purpose. Educational tool for teaching advanced control of power systems.
Date: November 2025
"""
...
def compute_droop_parameters(
    ...
    # Limiting power deliivered by generators to rated power
    # TO BE REVISED!!!
    if P1_s2 >= P_nom1:
        P1_s2 = P_nom1
    if P2_s2 >= P_nom2:
        P2_s2 = P_nom2
    if P_L2 >= P_nom1+P_nom2:
        P_L2 = P_nom1+P_nom2
...
- Specify two operating scenarios:
  - **Scenario 1**: the system operates at nominal frequency with adjustable power sharing between generators.
  - **Scenario 2**: the system operates at a frequency deviated from nominal,
    given in percent (e.g. -2% → 0.98 f_nom), with a selected power sharing between generators.
...
if use_example:
    ...
    frac_G1_s1_pct = 70   # 35 kW out of 50 kW for G1
...
if run_button:
    try:
        ...
        with col1:
            ...
            st.write(f"**Nominal frequency (Scenario 1)**: {results['f_nom']:.3f} Hz")
            st.write(
            ...
            st.write(f"**Power sharing for G1 in Scenario 1**: {results['frac_G1_s1_pct']:.1f} % of the system load")
            st.write(f"**Power sharing for G1 in Scenario 2**: {results['frac_G1_s2_pct']:.1f} % of the system load")
        ...
        for x in [P2_s1, P1_s1, P2_s2, P1_s2, P_load_nom, P1_s1+P2_s1, P1_s2+P2_s2]:
        ...
        ax.scatter([P2_s2], [f_op], color="black")
        ...
        ax.scatter([P1_s1+P2_s1], [f_nom], color="gray")
        ax.scatter([P1_s2+P2_s2], [f_op], color="gray")
        ...
        ax.text(P1_s1 + 1.0, f_nom + 0.15,
                f"({P1_s1:.1f} kW,\n {f_nom:.1f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")
        ax.text(P1_s1+P2_s1 + 1.0, f_nom + 0.15,
                f"({P1_s1+P2_s1:.1f} kW,\n {f_nom:.1f} Hz)",
                color="black", fontsize=8, ha="left", va="bottom")
        
         # Scenario 2 operating point for G1 and G2
        ax.text(P1_s2 + 1.0, f_op + 0.15,
                f"({P1_s2:.1f} kW,\n {f_op:.2f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")
        ax.text(P2_s2 + 1.0, f_op + 0.15,
                f"({P2_s2:.1f} kW,\n {f_op:.2f} Hz)",
                color="blue", fontsize=8, ha="left", va="bottom")
        ax.text(P1_s2+P2_s2 + 1.0, f_op + 0.15,
                f"({P1_s2+P2_s2:.1f} kW,\n {f_nom:.1f} Hz)",
                color="black", fontsize=8, ha="left", va="bottom")

        # Rated power frequency points
        ax.text(P_nom2 + 2.0, f2_Pnom - 0.3,
                f"({P_nom2:.1f} kW,\n {f2_Pnom:.2f} Hz)",
                color="blue", fontsize=8, ha="left", va="bottom")
        ax.text(P_nom1 + 2.0, f1_Pnom - 0.3,
                f"({P_nom1:.1f} kW,\n {f1_Pnom:.2f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")
