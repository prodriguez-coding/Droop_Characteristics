...
def compute_droop_parameters(
    ...
    f_min = min(f1_Pnom, f2_Pnom, f_op) - 0.2
...
st.set_page_config(
    page_title="Droop Control of Parallel Generators",
    layout="wide",
)

st.title("Droop Control of Two Synchronous Generators")
st.markdown(
    """
This tool illustrates **active power–frequency droop control** for two synchronous
generators operating in parallel and sharing the load.
...
- The app then computes:
  - The reference powers **P_ref1**, **P_ref2** at nominal frequency.
  - The droop gains **KPF1**, **KPF2** in kW/(rad/s).
  - The droop characteristics of both generators.
  - The **combined droop curve** (total power vs frequency).

Use this app as an interactive teaching tool: let students modify the parameters
and observe how the curves and operating points move.
"""
)
...
if use_example:
    ...
    P_L1_pct = st.sidebar.number_input(
        "Scenario 1 load level [% of rated load]", value=50.0, min_value=0.0, max_value=300.0
    )
    ...
    freq_err_pct = st.sidebar.number_input(
        ...
        value=-2.0,
        help="Example: -2 → f_op = 0.98 f_nom;  +1 → f_op = 1.01 f_nom.",
    )
...
if run_button:
    try:
        ...
        baseline = min(f1_Pnom, f2_Pnom, f_op) - 1.0
        ...
        # No-load labels
        ax.text(1.0, f1_P0 + 0.15, f"(0 kW, {f1_P0:.2f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")
        ax.text(1.0, f2_P0 - 0.25, f"(0 kW, {f2_P0:.2f} Hz)",
                color="blue", fontsize=8, ha="left", va="top")

        # Scenario 1 sharing at nominal frequency
        ax.text(P2_s1 + 2.0, f_nom + 0.15,
                f"({P2_s1:.1f} kW, {f_nom:.1f} Hz)",
                color="blue", fontsize=8, ha="left", va="bottom")
        ax.text(P1_s1 + 2.0, f_nom + 0.15,
                f"({P1_s1:.1f} kW, {f_nom:.1f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")

        # Scenario 2 operating point for G1
        ax.text(P1_s2 + 2.0, f_op + 0.15,
                f"({P1_s2:.1f} kW, {f_op:.2f} Hz)",
                color="black", fontsize=8, ha="left", va="bottom")

        # Rated power frequency points
        ax.text(P_nom2 + 2.0, f2_Pnom + 0.15,
                f"({P_nom2:.1f} kW, {f2_Pnom:.2f} Hz)",
                color="blue", fontsize=8, ha="left", va="bottom")
        ax.text(P_nom1 + 2.0, f1_Pnom + 0.15,
                f"({P_nom1:.1f} kW, {f1_Pnom:.2f} Hz)",
                color="red", fontsize=8, ha="left", va="bottom")
